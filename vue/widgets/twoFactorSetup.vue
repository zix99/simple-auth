<template>
  <div>
    ...
    <div v-if="secret" class="qrcode">
      <img :src="`api/v1/local/2fa/qrcode?secret=${secret}`" />
...
export default {
  ...
  methods: {
    activate() {
      this.loadingPromise = axios.post('api/v1/local/2fa', { secret: this.secret, code: this.code })
        .then(() => {
